...
class HumanRegisterController {
  ...
  Future<List<Human>> fetchHumanList() async {
    final response =
        await http.get(Uri.parse('http://192.168.1.35:8080/list/human'));
  ...
  Future createHumanCases(context) async {
    try {
      var response = await http.post(
        Uri.parse('http://192.168.1.35:8080/form/save'),
  ...
  Future editHumanCase(humanIdToEdit, context) async {
    try {
      var response = await http.put(
        Uri.parse('http://192.168.1.35:8080/edit/human/$humanIdToEdit'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
        },
        body: jsonEncode(<String, dynamic>{
  ...
  Future deleteHumanCase(humanIdToDelete, context) async {
    final response = await http.delete(
        Uri.parse('http://192.168.1.35:8080/delete/human/$humanIdToDelete'));
